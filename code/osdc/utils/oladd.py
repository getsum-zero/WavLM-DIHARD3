...
def overlap_add(tensor, function, window_size=1600 * 6, stride=1600 * 3, win_type='hann', pad_left=True,
    ...
    for i in range(1, n):
        l = max(0, i*stride - lookbehind)
        r = min(i*stride + window_size + lookahead, tensor.shape[-1])
        temp = np.copy(tensor[..., l:r])
        temp = function(temp)[..., lookbehind:window_size + lookbehind]
        temp *= window
